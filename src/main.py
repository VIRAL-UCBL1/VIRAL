...
def main():
    ...
    viral = VIRAL(
        env_type=env_type, model=model, hf=human_feedback, training_time=int(20_000),
        numenvs=1, options=additional_options)
    viral.generate_context(env_type.prompt)
    viral.generate_reward_function(n_init=3, n_refine=3)
